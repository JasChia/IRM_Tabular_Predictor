...
parser.add_argument('--hidden_dim', type =float, default=400)
...
class MLP(nn.Module):
    ...
    def __init__(self, n_inputs):
        ...
        layer1 = nn.Linear(n_inputs, flags.hidden_dim)
        layer2 = nn.Linear(flags.hidden_dim, 1)
...
def make_training_environment(path):
    ...
    np.random.shuffle(combined_data[0].numpy())
    np.random.set_state(rng_state)
    np.random.shuffle(combined_data[1].numpy())
...
def complete_table(avg_train_acc, avg_test_acc, avg_in_distribution_test_acc):
    rows = [['learning_rate', 'train_batch_size', 'max_epochs', 'erm_weight', 'irm_weight', 'training_acc', 'I_D_T_A', 'test_acc']]
    for n in range (0, 1):
        rows.append((str(flags.lr), str(flags.training_batch_size), \
    ...
    for line in pretty_table(rows, 8):
        print (line)
...
for b in range(0, flags.n_restarts):
    ...
    # data file paths
    test_files = ["France_DL.csv"]
    training_files = ["Germany_DL.csv", "USA_DL.csv", "Austria_DL.csv", "China_DL.csv"]
    ...
    total_in_distribution_test_acc += evaluate_model(in_distribution_test_set, model)
...
avg_train_acc = avg_train_acc/flags.n_restarts
avg_test_acc = avg_test_acc/flags.n_restarts
...
#printing results
print("File name:", file_name, "Run time:", run_time)
